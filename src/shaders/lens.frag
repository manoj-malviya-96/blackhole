...
vec3 starField(vec3 dir, float uTime) {
    ...
    if (h > 0.988) {
        ...
        star = tint * smoothstep(0.45, 0.0, d) * brightness;
    }
    return star;
}

// Cheap non-volumetric nebula: a drifting fbm haze wrapped around the sky
// direction, standing in for wispy gas clouds without a full raymarch.
vec3 gasCloud(vec3 dir, float uTime) {
    vec2 uv = vec2(atan(dir.z, dir.x), dir.y);
    float n = fbm(uv * 2.5 + vec2(uTime * 0.015, uTime * 0.01));
    float density = smoothstep(0.45, 0.85, n);
    return vec3(0.30, 0.40, 0.62) * density * 0.30;
}
